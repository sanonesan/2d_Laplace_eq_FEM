...
template<typename T>
void scheme_2d_Laplace_equation_mod(
    ...
    */
    ...
    for(std::size_t i = 0; i < full_stiffness_matrix.get_rows(); ++i){
        for(auto it = ind_left_n_right_boundary_correlation.begin(); it != ind_left_n_right_boundary_correlation.end(); ++it){
            ...
            full_stiffness_matrix[i][(*it).second] = 0.;
        }
    }
    ...
    for(auto it = ind_left_n_right_boundary_correlation.begin(); it != ind_left_n_right_boundary_correlation.end(); ++it){
        for(std::size_t i = 0; i < full_stiffness_matrix.get_cols(); ++i){
            //if (i != (*it).first && i != (*it).second){
                full_stiffness_matrix[(*it).second][i] = 0.;
            //}
        }
        full_stiffness_matrix[(*it).second][(*it).first] = 1.;
        full_stiffness_matrix[(*it).second][(*it).second] = -1.;
    ...
    */
    ...
    reduced_sys_size = laplace_eq._nodes.size(); // - laplace_eq._right_boundary_nodes.size(); //- laplace_eq._dirichlet_upper_boundary_nodes.size() - laplace_eq._dirichlet_lower_boundary_nodes.size() 
