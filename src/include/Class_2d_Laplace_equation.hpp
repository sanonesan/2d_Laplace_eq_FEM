...
class Class_2d_Laplace_equation{
    ...
    public:
        ...
        std::vector<std::vector<T>> _dirichlet_left_boundary_edges;
        std::vector<std::vector<T>> _dirichlet_right_boundary_edges;
        std::vector<std::vector<T>> _dirichlet_upper_boundary_edges;
        std::vector<std::vector<T>> _dirichlet_lower_boundary_edges;
        ...
        Class_2d_Laplace_equation<T> DEFAULT_TEST(){
            ...
            //Reading edges of boundary conditions 

            // fin.open("meshes/mesh/boundaries/edges/mesh_left_boundary_edges.txt");
            // if (!fin.is_open()) {
            //     throw std::invalid_argument("Smth's wrong with path");
            // }            
