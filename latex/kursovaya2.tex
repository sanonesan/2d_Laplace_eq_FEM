...
\begin{document}
	...
	\subsection{Задача №\,2 (периодические граничные условия и граничные условия первого рода)}
		\subsubsection{Условие задачи}
			\looser{0.01}{Найти потенциал в области $\Omega = \left\{ -\infty \leqslant x \leqslant \infty, \  w(x) \leqslant y \leqslant 2 \right\}$}, где $w(x)$ --- периодическая функция с периодом $T = 2$, такая что $w(x) = |x - 1| + 1, \forall x \in \left[ 0, 2 \right]$. 
