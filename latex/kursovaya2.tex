		...
		\subsection{Метод конечных элементов на треугольной сетке}
			...
			\subsubsection{Применение граничных условий первого рода}
				...
				\begin{equation}
					...
					\label{sys_in_index}
				\end{equation}
				где $a_{ij}$ --- компоненты матрицы $A$, $b_{j} \ u_{j}$ --- компоненты вектора правой части $b$ и вектора неизвестных $u$ соответственно. Значения решения $u$ в узлах $P_k \in \Gamma_D$ известны и равны $u_k = g(P_k), \forall P_k \in \Gamma_D$. Тогда уравнения (\refeq{sys_in_index}) могут быть перезаписаны следующим образом
				\begin{equation}
			...
			\subsubsection{Применение периодических граничных условий}
				...
				\end{itemize} 
				В результате размерность решаемой системы уменьшилась на единицу, а условие $u_p = u_q$ будет применено уже к итоговому решению \cite{Stahel}.
