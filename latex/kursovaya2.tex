		...
		\subsection{Метод конечных элементов на треугольной сетке}
			...
			\subsubsection{Применение граничных условий первого рода}
				...
				\begin{equation}
					...
					\label{sys_in_index}
				\end{equation}
				где $a_{ij}$ --- компоненты матрицы $A$, а $b_{j}$, и $u_{j}$ --- компоненты вектора правой части $b$ и вектора неизвестных $u$ соответственно. Значения решения $u$ в узлах $P_k \in \Gamma_D$ известны и равны $u_k = g(P_k), \forall P_k \in \Gamma_D$. Тогда уравнения (\refeq{sys_in_index}) могут быть перезаписаны следующим образом
				\begin{equation}
				...
				\end{equation}
				где, как было указано выше, $I$ --- множество индексов, узлов лежащих внутри области, $|I| = M < N$. Что приводит к уменьшению размера  матрицы $A$ от $N \times N$ до $M \times M$, по средствам удаления строк и столбцов с номерами $k \not \in I$ \cite{Galanin}.
			...
			\subsubsection{Применение периодических граничных условий}
				...
				\looser{0.02}{Пусть на узлы $P_p$ и $P_q$ наложено условие периодичности, т.~е.} $$u_p = u (P_p) = u(P_q) = u_q.$$ Тогда, чтобы учесть периодичность нужно изменить систему, полученную на предыдущем этапе. Нужно внести следующие изменения:
				\begin{itemize}
				...
				\end{itemize} 
				\looser{-0,009}{В результате размер решаемой системы уменьшился на единицу, а условие $u_p = u_q$} будет применено уже к итоговому решению \cite{Stahel}. Таким образом рассматриваются все узлы, на которые наложено условие периодичности.
