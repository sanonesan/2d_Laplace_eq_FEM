...
\begin{document}
	...
	\section{Решение двумерного уравнения Лапласа}
		...
		\section{Примеры решения задачи}
			...
			\subsubsection{Оценка погрешности аппроксимации решения}
				Для оценки погрешности полученного решения $u_{approx}$ относительно точного решения $u_{exact}$ возьмем среднее значении абсолютной погрешности по всем узлам (\ref{abs_err}) и погрешность решения в норме $L_2$ (\ref{l2_err}):
				\begin{equation}
					\mathrm{AbsErr} = \dfrac{1}{N} \sum_{i = 1}^{N} \left| u_{exact, i} - u_{approx, i} \right|,
					\label{abs_err}
				\end{equation}
				\looser{0.02}{где $N$ --- количество узлов, $u_{exact, i},\ u_{approx, i}$ --- точное и приближенное решения в $i$-ом узле;}
				\begin{equation}
					\mathrm{Err}_{L_2} = || u_{exact} - u_{approx} ||_{L_2} = \int_S (u_{exact} - u_{approx})^2 dS,
					\label{l2_err}
				\end{equation}
				где $S$ --- площадь области $\Omega$. Численная аналог формулы (\refeq{l2_err}) получается следующим образом:
				\begin{gather*}
					...
					u_{approx,\,ij} = \sum_{k = 1}^{N_v} u_{apсодержимое...prox,\,ij} \, \phi_k,		
				\end{gather*} 
				\looser{0.0115}{где 
					где $N_{el}$ --- количество конечных элементов,
					$S_i$ --- площадь $i$-ого конечного элемента, $N_v$ --- количество узлов конечного элемента, $u_{exact,\,ij},\ u_{approx,\,ij}$ --- точное и приближенное значения решения в $j$-ом узле $i$-ого элемента, $\phi_k$ --- базисная функция в $k$-ом узле $i$-ого кочечного элемента.} 
				...
				\begin{equation}
					\mathrm{Err}_{L_2} \approx \sum_{i = 1}^{N_{el}} S_i
		...
		\begin{table}[!h]
			...
			\begin{tabular}{|c|c|c|c|}
				...
				& \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Погрешность решения\\ в норме $L_2$\end{tabular}} \\
	...
	\subsection{Задача №\,2 (периодические граничные условия и граничные условия первого рода)}
		\subsubsection{Условие задачи}
			\looser{0.01}{Найти потенциал в области $\Omega = \left\{ -\infty \leqslant x \leqslant \infty, \  w(x) \leqslant y \leqslant 2 \right\}$}, где $w(x)$ --- периодическая функция с периодом $T = 2$, такая что $w(x) = |x - 1| + 1, \forall x \in \left[ 0, 2 \right]$, на верхней границе потенциал равен 0 В, на нижней --- 10 В.
		\subsubsection{Решение}
			...
			\subsubsection{Проверка решения на удовлетворение условиям периодичности}
				...
				\begin{enumerate}
					...
					\begin{table}[!h]
						\centering
						\caption{Сравнение значений численного решения в первой группе точек}
						\vspace*{2mm}
						\begin{tabular}{|c|c|c|}
							\hline
							Область решения
					...
					\begin{table}[!h]
						\centering
						\caption{Сравнение значений численного решения во второй группе точек}
						\vspace*{2mm}
						\begin{tabular}{|c|c|c|}
							\hline
							Область решения
				...
				\looser{0.02}{Из таблиц видно, что значения отличаются примерно на порядок $O(h^2)$, что} показывает, что решение удовлетворяет условиям периодичности.
				
			
	\newpage
	\subsection{Задача №\,3 (периодические граничные условия и граничные условия первого рода)}
	\subsubsection{Условие задачи}
		...
		%\vspace{5.0mm} 
			{ 
				\begin{minipage}{0.47\textwidth} 
					\centering 
					\hspace*{-20mm}
					\includegraphics[width=1.4\columnwidth]{Test_domain_1_1_sin_mesh_0001_calfem_net_1.png}\\
					\textit{a}
			...
			{ 
				\begin{minipage}{0.47\textwidth} 
					\centering 
					\hspace*{-10.mm}
					\includegraphics[width=1.4\columnwidth]{Test_domain_1_1_sin_mesh_0001_calfem.png}\\
					\textit{b}
	...
	\subsubsection{Проверка решения на удовлетворение условиям периодичности}
		Аналогично предыдущей задаче рассмотрим две группы точек:
		\begin{enumerate}
			
			\item Точки вида: $(1, y),\ (5, y)$ --- для решения построенного на одном периоде функции $w(x)$,
			$(1, y),\ (5, y),\ (9, y),\ (13, y)$ --- для решения построенного на трех периодах функции $w(x)$ (таблица ).
			
			\item Точки вида $(1, y)$ --- для решения построенного на одном периоде функции $w(x)$,
			\looser{-0.02}{$(1, y),\ (3, y),\ (5, y)$ --- для решения построенного на трех периодах функции $w(x)$ (таблица ).}
	...
	\section{Программная реализация алгоритма}
		...
		Построение сеток --- Wolfram Mathematica и CALFEM, алгоритм метода конечных элементов реализован на языке C++
