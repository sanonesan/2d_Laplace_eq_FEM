...
\begin{document}
	...
	\section{Решение двумерного уравнения Лапласа}
		...
		\subsection{Аппроксимация уравнения Лапласа}
			...
			\looser{-0.02}{Для аппроксимации задачи с помощью метода конечных элементов (МКЭ) рассмотрим конечномерное пространство $V_h$, аппроксимирующее пространство $V$ и пространство 
			...
			Пусть $\phi_i$ --- базис в пространстве $V_h$, $i = \overline{1, N},$ причем часть функций $\phi_i$ с номерами $i \in I$ образуют базис в пространстве $V_{D,h}$, т.е. обращаются в ноль на границе $\Gamma_D$. Количество таких индексов будем считать равным $M = |I| < N,\ |I| > 1$.
			...
			\looser{-0.02}{Представляя неизвестное решение в виде линейной комбинации базисных функций:}
			\begin{equation*}
		...
		\subsection{Метод конечных элементов на треугольной сетке}
			...
			\subsubsection{Наложение периодических граничных условий}
				...
				\end{itemize} 
				\looser{-0,009}{В результате размер решаемой системы уменьшится на единицу, а условие $u_p = u_q$} будет применено уже к итоговому решению \cite{Stahel}. Таким образом рассматриваются все узлы, на которые наложено условие периодичности. В данной работе был применен алгоритм с сохранением симметричности матрицы жесткости.
